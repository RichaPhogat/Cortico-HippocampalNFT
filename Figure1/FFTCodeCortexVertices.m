...
plotColor = [0.5, 0.6, 1.0];
    ...
    for jk = 1:size(corticalEigenvectors, 1)
        yy = corticalEigenvectors(jk, 2:numModesC) * modeCoeffPhiF(2:end, :);
...
save('FFTCortex.mat', 'FFTCortex', 'freqAxis', '-v7.3')
...
function powerSpectrum = calculatePowerSpectrum(signal,Fs)
    ...
    powerSpectrum(2:end-1) = 2 * powerSpectrum(2:end-1)/(n*Fs); % Normalize
end