function dW = generateNoiseCH(tcount, dt, numModesC, numModesH, ...
    ...
    case 'eigenmode'
        ...
        for eigValH = 1:numModesH
            noiseHippo = noiseHippo + hippoEigenvectors(:,eigValH)*randn(1,tcount);
        end

        noiseCortex = zeros(mC, tcount);
        for eigValC = 1:numModesC
            noiseCortex = noiseCortex + corticalEigenvectors(:,eigValC)*randn(1,tcount);
        end
