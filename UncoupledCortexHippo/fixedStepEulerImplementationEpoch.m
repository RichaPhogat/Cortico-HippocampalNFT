function [sol, params] = fixedStepEulerImplementationEpoch(sdefun, sdegun, sol, tcount, delaytimeC,...
    ...
    for indx=1:tcount-1
        ...
        if delaytimeC ~= 0
            if indx - delaytimeC > 0
                ...
                yModeCortexDelayed = sol.y(1 : 2*numModesC, end + indx - delaytimeC - 1);
                yCortexDelayed = sol.y(2*numModesC + 2*numModesH + 6*nodesH + 1 : 2*numModesC...
                    + 2*numModesH + 6*nodesH + 6*nodesC, end + indx - delaytimeC - 1);
                yModeHippoDelayedC = sol.y(2*numModesC + 1 : 2*numModesC + 2*numModesH,...
                    end + indx - delaytimeC - 1);
            end
        else
            yCortexDelayed = sol.y;
        end
        ...
        if delaytimeH ~= 0
            if indx - delaytimeH > 0
                ...
                yModeHippoDelayed = sol.y(2*numModesC + 1 : 2*numModesC + 2*numModesH,...
                    end + indx - delaytimeH - 1);
                yHippoDelayed = sol.y(2*numModesC + 2*numModesH + 1 : 2*numModesC...
                    + 2*numModesH + 6*nodesH, end + indx - delaytimeH - 1);
            end
        else
            yCortexDelayed = sol.y;
        end
